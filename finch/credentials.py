...
class CreateCredentialWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Create Credential")
        self.resize(500, 200)
        center_window(self)
        ...
        endpoint_label = QLabel("Service Endpoint:")
        self.endpoint_input = QLineEdit("https://s3.amazonaws.com")
        access_key_label = QLabel("Access Key:")
        self.access_key_input = QLineEdit()
        access_secret_label = QLabel("Secret Key:")
        self.access_secret_input = QLineEdit()
        region_label = QLabel("Region:")
        self.region_input = QLineEdit("us-east-1")
        create_button = QPushButton("Create Credential")
        create_button.setIcon(QIcon(resource_path('img/plus.svg')))
        create_button.clicked.connect(self.create_credential)

        layout.addWidget(name_label, 0, 0)
        layout.addWidget(self.name_input, 0, 1)
        layout.addWidget(endpoint_label, 1, 0)
        layout.addWidget(self.endpoint_input, 1, 1)
        layout.addWidget(access_key_label, 2, 0)
        layout.addWidget(self.access_key_input, 2, 1)
        layout.addWidget(access_secret_label, 3, 0)
        layout.addWidget(self.access_secret_input, 3, 1)
        layout.addWidget(region_label, 4, 0)
        layout.addWidget(self.region_input, 4, 1)
        layout.addWidget(create_button, 5, 0)

        self.setLayout(layout)

    def create_credential(self):
        credential_info = {"name": self.name_input.text(), "endpoint": self.endpoint_input.text(),
                           "access_key": self.access_key_input.text(), "region": self.region_input.text()}
        key_map = {'name': 'Credential Name', 'endpoint': 'Service Endpoint', 'access_key': 'Access Key',
                   'secret_key': 'Secret Key', 'region': 'Region'}
        try:
            for key in credential_info:
                if credential_info[key].strip() == '':
                    raise ValueError(f"Field {key_map[key]} cannot be empty")
            self.credentials_manager.append_credentials_file(credential_info)
            keyring.set_password(f"{slugify(self.name_input.text())}@finch",
                                 self.access_key_input.text(),
                                 self.access_secret_input.text())
            self.close()
